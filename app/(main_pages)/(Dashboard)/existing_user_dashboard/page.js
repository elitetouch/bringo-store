...
import { Card_data } from "@/app/component/existing_user_dashboard_components/Card_data";
import Card from "@/app/component/existing_user_dashboard_components/Card";
import { SellingProduct } from "@/app/component/existing_user_dashboard_components/SellingProductData";
import Image from "next/image";
import MobileTable from "@/app/component/Table/MobileTable";
import { useRouter, useSearchParams } from "next/navigation";
import { ProfileInfo } from "@/app/api/reactQuery";
import AddButton from "@/app/component/existing_user_dashboard_components/AddButton";
import { useStore } from "@/app/component/Store/useStore";
import { NewOutlet } from "@/app/component/existing_user_dashboard_components/NewOutletModal";
...
export const ExistingDashboardHeader = ({ profile, setOpen }) => {
  ...
  return (
    <Box className=" w-11/12 m-auto lg:flex grid gap-y-[20px] justify-between items-center pt-[20px] lg:pt-[30px]">
      ...
      Please, complete your store set up to go live!
    </Text> */}
          <Box>
            <Text className=" text-[15px] mt-[10px] text-[#FF4F4A]">
              Please, complete your verification to go live!
            </Text>
            <Text
              as={"button"}
              color={"#47A3FF"}
              ...
              className=" text-[12px] mt-[10px] text-[#47A3FF] italic"
            >
      ...
      <Box className=" flex items-center gap-x-[20px]">
        <AddButton
          text="Create outlet"
          onClick={() => {
            // do something else, e.g. open modal
            setOpen(true);
          }}
        />
        <AddButton
...
//import imp from '../KycVerification'
function Page() {
  const router = useRouter();
              ...
              <Box className="">
                <Box>
                  <Box
                    ...
                    className=" flex items-center justify-between pb-[10px] w-11/12 m-auto"
                  >
                    <Box>
                      <Text className=" lg:text-[18px] font-semibold text-[12px]">
                  ...
                  {SellingProduct.length > 0 ? (
                    SellingProduct.map((item) => {
                      return (
                        <Box
                          key={item.id}
                          className=" flex items-center gap-x-[20px] w-full"
                        >
                          <Box>

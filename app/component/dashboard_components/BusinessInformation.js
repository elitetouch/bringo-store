...
const [err, setErr]= useState({})
    const Validation = () =>{  
      ...
      const objectKeys=[ 'companyName','certificate','fullName','idNumber','idType','logo'
         ]
    ...
    const handlePaymentDetailsSubmit=()=>{
      ...
      if(Validation()){
       ...
       paymentData?.companyName && formData.append('company_name',paymentData.companyName)
       paymentData?.taxNumber && formData.append('tin',paymentData.taxNumber)
      paymentData?.cao &&  formData.append('cac02',paymentData.cao)
      paymentData?.certificate &&  formData.append('cac_certificate',paymentData.certificate)
      paymentData?.logo &&  formData.append('company_logo',paymentData.logo)
      paymentData?.fullName &&  formData.append('rep_fullname',paymentData.fullName)
      paymentData?.idNumber &&  formData.append('rep_id_number',paymentData.idNumber)
      paymentData?.idType &&  formData.append('rep_id_type',paymentData.idType)
          axiosInstance
  ...
  headers: {
    ...
    fullName:'',
     ...
     suscribeStatus === "inactive"?router.push('../Subscription'): setFormPage(0)  
        })
        .catch((error) => {
          setPaymentLoader(false);
          toast({
      ...
      <Box className={`${mobilePage===0?' grid pb-[20px] lg:pb-0':'hidden lg:grid'}`}>
       <Box borderBottom="2px" borderColor="#92DF16"  className=' w-fit'>
                      ...
                      <Text className=' text-[#332F2F] text-[12px] pt-[15px]'>Please provide the following details of your business</Text>
                       ...
                       <DashBoardInput
                       required
                       placing={Object?.keys(SingleBussinessData).length > 0?SingleBussinessData.companyName:'Nakasero'} icon={<svg width="22" height="20" viewBox="0 0 22 20" fill="none" xmlns="http://www.w3.org/2000/svg">
 ...
 {err?.taxNumber? (
  <p className="text-red-600 text-[12px] pt-[5px]">Please input Tax identification number</p>
                        ...
                        <Box>
                         <DashboardFileUpload
                         required
                         UploadedFileName={Object?.keys(SingleBussinessData).length > 0?SingleBussinessData.companyLogo:''} attachFile={setPaymentData} names={'logo'} label={'Company logo'} />
                           {err?.logo && (
        ...
        <Box borderBottom="2px" borderColor="#92DF16"  className=' w-fit pt-[30px]'>
                       ...
                       <Text className=' text-[#332F2F] text-[12px] pt-[15px]'>Please provide the following details of the owner / legal representative of your business</Text>
                        <Box className=' grid lg:grid-cols-2 gap-y-[20px] lg:gap-x-[40px] mt-[20px] lg:mt-[20px]'>
                          ...
                          <DashBoardInput
                          required
                          placing={Object?.keys(SingleBussinessData).length > 0?SingleBussinessData.repFullname:'Naruto Uzumaki'} 
names={'fullName'} 
values={paymentData.fullName}
 handleChange={handlePaymentChange}
  ...
  <p className="text-red-600 text-[12px] pt-[5px]">Please select an appropriate ID type</p>
                 ...
                 <DashBoardInput
                 required
                 placing={Object?.keys(SingleBussinessData).length > 0?SingleBussinessData.repIdNumber:'Enter ID number'} 
names={'idNumber'} 
